...
//       try {
//         // ✅ Fetch camp details and set camp_name
//         const camp = await this.reliefCampService.getCampById(this.campId);
//         if (camp) {
//           this.resident.camp_id = this.campId; // Ensure camp_id is set
...
//       // ✅ Check if it's in edit mode and load resident data
...
//       if (this.isEditMode && this.residentId) {
//         await this.reliefCampService.updateCampMember(this.residentId, residentData);
//       } else {
//         await this.reliefCampService.addCampMember(residentData);
//       }
//       this.goBack();
//     } catch (error) {
...
@Component({
  ...
  template: `
    <div class="resident-form-container">
      <div class="resident-form-header glass">
        <div class="container">
          <h1>{{ isEditMode ? 'Edit Resident Details' : 'Add New Resident' }}</h1>
          <p>{{ isEditMode ? 'Update existing resident information' : 'Register a new resident for ' + resident.camp_name }}</p>
        </div>
      </div>
      ...
      <div class="container resident-form-content">
        <div class="resident-form-card card">
          <form (ngSubmit)="onSubmit()" #residentForm="ngForm">
            ...
            <!-- Camp details are now read-only and automatically populated -->
            <div class="form-group">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <div *ngIf="contactNumber.invalid && contactNumber.dirty" class="error-message">
  ...
  styles: [`
    ...
    .form-input[readonly] {
      background-color: rgba(136, 112, 112, 0.05);
      cursor: not-allowed;
      border-color: rgba(136, 112, 112, 0.1);
    }
...
export class CampResidentFormComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      if (!this.campId) {
        this.handleError('No camp ID provided. Please select a camp first.', '/volunteer/camps');
        return;
      }
  ...
  async loadCampDetails() {
    try {
      ...
      if (camp) {
        ...
        this.handleError('Camp not found!');
      }
    } catch (error) {
      console.error('Error loading camp details:', error);
      this.handleError('Failed to load camp details. Please try again.');
    }
  }
  ...
  async loadResident(id: string) {
    try {
      const resident = await this.reliefCampService.getResidentById(id);
      if (resident) {
        this.resident = { ...resident, arrival_date: new Date(resident.arrival_date).toISOString().split('T')[0] };
      } else {
        this.handleError('Resident not found');
      }
    } catch (error) {
      console.error('Error loading resident:', error);
      this.handleError('Failed to load resident details');
    }
  }
  ...
  async onSubmit() {
    ...
    try {
      const residentData = { ...this.resident, camp_id: this.campId };

      if (this.isEditMode && this.residentId) {
        await this.reliefCampService.updateCampMember(this.residentId, residentData);
      } else {
        await this.reliefCampService.addCampMember(residentData);
        await this.updateCampCount(1); // Increase the count by 1
  ...
  async updateCampCount(change: number) {
    try {
      const camp = await this.campservice.getCampById(this.campId!);
      if (camp) {
        const updatedCount = (camp.curr_count || 0) + change;
        await this.reliefCampService.updateCamp(this.campId!, { curr_count: updatedCount });
      }
    } catch (error) {
      console.error('Error updating camp count:', error);
      this.errorMessage = 'Failed to update camp resident count.';
    }
  }
  
  goBack() {
    this.router.navigate(this.campId ? ['/volunteer/camps', this.campId, 'residents'] : ['/volunteer/camps']);
  }

  handleError(message: string, redirectPath: string = '/volunteer/camps') {
