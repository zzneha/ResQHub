...
interface Camp {
  ...
  name: string;
  location: {
    address: string; // Ensure address is part of the location object
    latitude?: number;
    longitude?: number;
  };
  capacity: number;
      ...
      <div class="container dashboard-content">
        <div class="dashboard-sidebar">
          ...
          <div class="nav-card card">
            ...
            <a routerLink="/volunteer/reportsr" routerLinkActive="active" class="nav-link">Recent Incidents</a>
        ...
        <div class="dashboard-main">
          ...
          <div class="quick-actions card">
            ...
            <div class="action-buttons">
              ...
              <a routerLink="/volunteer/reportsr" class="btn btn-secondary">Recent Incidents</a>
            </div>
          </div>
            ...
            <div *ngIf="camps.length > 0" class="camps-grid">
              <div *ngFor="let camp of camps" class="camp-card glass-hover">
                <div class="camp-header">
                  <h4>{{ camp.name }}</h4>
                  <span class="resident-count">{{ camp.curr_count || 0 }} residents</span>
                </div>
                <p class="camp-location">📍 {{ camp.location.address }}</p>
                <div class="camp-capacity">
                  ...
                  <span class="capacity-text">
                    {{ camp.curr_count || 0 }}/{{ camp.capacity }} capacity
                  </span>
                </div>
...
export class DashboardComponent implements OnInit {
  ...
  constructor(
    ...
    private reliefcampserve: ReliefCampService,
  ...
  async loadData() {
    try {
      ...
      const camps = await this.reliefcampserve.getCamps(this.currentUser.user_id);
      this.camps = await Promise.all(camps.map(async (camp: Camp) => {
        const residents = await this.reliefcampserve.getResidents(camp.id);
        return {
          ...camp,
      ...
      this.memberCount = this.camps.reduce((total, camp) => total + (camp.curr_count || 0), 0);
    } catch (error) {
  ...
  getCapacityPercentage(camp: Camp): number {
    if (!camp.capacity || !camp.curr_count) return 0;
    return Math.min((camp.curr_count / camp.capacity) * 100, 100);
  }
